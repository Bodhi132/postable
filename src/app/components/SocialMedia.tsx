...
interface SocialMediaConfig {
  ...
  bgColor: string;
  imgSrc: string;
  initial: { x: string; rotate: string; y: string };
  animate: { x: string; y: string; rotate: string };
}
        ...
        <div className="mx-auto flex w-full max-w-6xl px-4 xl:pe-4 xl:ps-0">
          {socialMediaConfigs.map(({ name, bgColor, imgSrc, initial }, index) => (
            <motion.div
              ...
              ref={refs[index]}
            >
<h4 className={`flex items-center gap-2 rounded-t-3xl px-4 py-4 text-lg font-medium md:rounded-t-[50px] md:px-12 md:py-8 `} style={{backgroundColor:bgColor}}>
